...
export default function Pricing({ countryCode = 'KE' }: PricingProps) {
    ...
    const tiers = {
        ...
        growth: {
            ...
            desc: "Optimized for established enterprises aiming to dominate their industry digital footprint.",
        ...
        commerce: {
            ...
            features: ['Everything in Growth', 'Full eCommerce Infrastructure', 'Local & Global Payment Gateways', 'CRM & Inventory Pipeline Sync', 'Advanced Schema Optimization', '2 Months Priority Technical Support']
        }
    };
            ...
            <div className="mt-8 grid gap-6 md:mt-20 md:grid-cols-3 items-stretch">
                <Card className="flex flex-col justify-between h-full">
                        ...
                        <ul className="list-outside space-y-3 text-sm">
                            {tiers.starter.features.map((item, index) => (
                                <li key={index} className="flex items-center gap-2">
                                    <Check className="size-3 text-webmeka shrink-0" />
                    ...
                    <CardFooter className="mt-auto font-mono">
                        <Button asChild variant="outline" className="w-full rounded-full hover:border-webmeka hover:border-2">
                            <Link href="#contact">Get Started</Link>
                        </Button>
                    </CardFooter>
                </Card>

                <Card className="relative flex flex-col justify-between h-full border-webmeka/40">
                    <GridBackground />
                    <span className="absolute inset-x-0 -top-3 mx-auto flex h-6 w-fit items-center rounded-full px-3 py-1 bg-webmeka text-xs font-semibold text-neutral-900 ring-2 ring-webmeka ring-offset-2 ring-offset-card">Popular</span>
                    ...
                    <CardFooter className="mt-auto font-mono">
                        <Button asChild className="w-full border-webmeka border-2 bg-transparent text-neutral-300 rounded-full hover:bg-webmeka hover:text-neutral-900 transition-all duration-200">
                            <Link href="#contact">Get Started</Link>
                        </Button>
                    </CardFooter>
                </Card>
